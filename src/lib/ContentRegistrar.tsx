...
function ContentRegistrar() {
  ...
  { identificador_TipoGenero: number; descripcion: string }[]
>([]);
  const [tiposDocumento, setTiposDocumento] = useState<
  { identificador_TipoDocumentoIdentificativo: number; nombre_TipoDocumentoIdentificativo: string }[]
>([]);
  ...
  const handleChangeGenero = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const tipoGeneroValue = parseInt(selectedGeneroDesc, 10); // Convert to number
    setFormData({
      ...formData,
      genero: selectedGenero,
      tipoGenero: tipoGeneroValue, // Assign the converted value
    });
  ...
  const handleChangeTipoDoc = (event: React.ChangeEvent<HTMLSelectElement>) => {
    const selectedTipoDoc = parseInt(event.target.value, 10);
    setFormData({
      ...formData,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (formData && (formData as RegistrationFormData).nombre.trim().length < 3 || (formData as RegistrationFormData).nombre.trim() === "") {
    ...
    try {
      ...
      const response: RegisterResponse = { isSuccess: false, message: "Error al enviar la información. Por favor, inténtalo de nuevo más tarde." };
      ...
      try {
        ...
        if (token !== null) {
          ...
          if (documentos !== null && generos !== null) {
            const documentosConOpcionVacia = [
